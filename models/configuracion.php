...
class Configuracion {
    ...
    // Eliminar fondo de una vista específica
public function eliminarFondo($vista) {
    // Obtener ruta actual
    $fondo = $this->obtenerFondo($vista);
    ...
    // Actualizar BD para dejar el campo vacío
    $sql = "UPDATE configuracion SET fondo = '' WHERE vista = :vista";
    $stmt = $this->conn->prepare($sql);
    $stmt->bindParam(':vista', $vista);
    return $stmt->execute();
}
